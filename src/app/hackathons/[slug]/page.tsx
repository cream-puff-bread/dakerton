...
export default function HackathonDetailPage({
  ...
  const isSubmissionClosed = useMemo(() => {
    ...
    return (
      new Date(hackathon.period.submissionDeadlineAt).getTime() < Date.now()
    );
  }, [hackathon]);
  ...
  const btnPrimary =
    'text-[13px] px-4 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-opacity';
  const btnSecondary =
        ...
        >
          ...
          {isEnded && (
            <div className="flex items-center gap-2 mb-4 px-4 py-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-sm text-muted-foreground">
              <AlertTriangle size={14} />이 해커톤은 종료되었습니다.
            </div>
          )}
              ...
              (s.prize?.items ? (
                <div className="space-y-4">
                  ...
                  <div className={cardCls}>
                    ...
                    <p className="text-2xl font-bold text-primary">
                      {formatKRW(
                        s.prize.items.reduce((acc, p) => acc + p.amountKRW, 0),
                      )}
                    </p>
                  </div>
                </div>
